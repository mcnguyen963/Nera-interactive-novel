...
export function ParagraphBlock({ chapterIndex, paragraphIndex, paragraph }: ParagraphBlockProps) {
  ...
  const { updateParagraph, deleteParagraph } = useStoryStore()
    ...
    >
      {editing ? (
        <textarea
          ...
          autoFocus
          className="w-full bg-[var(--highlight)] outline-2 outline-[rgba(107,58,31,0.25)] rounded-[3px] p-1 text-[var(--ink)] font-[var(--font-body)] text-[1rem] leading-[1.85] resize-none"
        />
      ) : (
        <p
          onDoubleClick={() => { setEditing(true); setEditText(paragraph.text) }}
          className={`text-[1rem] leading-[1.85] py-1 outline-none rounded-[2px] transition-[background] duration-100 whitespace-pre-wrap break-words ${
      ...
      {paragraph.images.length > 0 && (
        <div className="flex gap-2 my-2 flex-wrap">
          {paragraph.images.map((url, i) => (
            <img key={i} src={url} alt="Generated" className="max-w-[200px] rounded-[var(--r)]" />
          ))}
        </div>
      )}
