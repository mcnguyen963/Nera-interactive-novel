...
export function Button({ variant = 'default', children, className = '', ...props }: ButtonProps) {
  ...
  const secondary = 'bg-[var(--page)] text-[var(--ink3)] border-[var(--rule)] hover:border-[var(--accent)] hover:text-[var(--accent)]'
  const cls = `${base} ${variant === 'primary' ? primary : variant === 'secondary' ? secondary : ''} ${className}`
